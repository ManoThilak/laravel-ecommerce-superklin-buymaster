...
class SystemController extends Controller
{
    ...
    {
        $new_withdraw = DB::table('withdraw_requests')->where(['checked' => 0])->whereNull('delivery_man_id')->count();
        return response()->json([
            'success' => 1,
            'data' => ['new_withdraw' => $new_withdraw]
        ]);
    }

    public function withdrawd_data()
    {
        $new_withdraw = DB::table('withdraw_requests')->where(['checked' => 0])->whereNotNull('delivery_man_id')->count();
        return response()->json([
            'success' => 1,
            'data' => ['new_withdraw' => $new_withdraw]
        ]);
