...
<audio id="myAudiod">
    <source src="{{asset('public/assets/back-end/sound/notificationd.mp3')}}" type="audio/mpeg">
</audio>
<script>
...
<script>
    ...
    @if(\App\CPU\Helpers::module_permission_check('order_management'))
        setInterval(function () {
            $.get({
                url: '{{route('admin.get-withdraw-data')}}',
                dataType: 'json',
                success: function (response) {
                    let data = response.data;
                    if (data.new_withdraw > 0) {
                        playAudio();
                        $('#popup-modal-withdraw').appendTo("body").modal('show');
                    }
                },
            });
        }, 30000);
    ...
    function check_withdraw() {
        location.href = '{{route('admin.sellers.withdraw_list')}}';
    }
    ...
    @if(\App\CPU\Helpers::module_permission_check('order_management'))
        setInterval(function () {
            $.get({
                url: '{{route('admin.get-withdrawd-data')}}',
                dataType: 'json',
                success: function (response) {
                    let data = response.data;
                    if (data.new_withdraw > 0) {
                        playAudiod();
                        $('#popup-modal-withdrawd').appendTo("body").modal('show');
                    }
                },
            });
        }, 30000);
    ...
    function check_withdrawd() {
        location.href = '{{route('admin.delivery-man.withdraw-list')}}';
    }
</script>
<script>
